class VendingMachine
  ...
  def possible_to_buy
    possible_to_buy = []
    @juice.each do |juice|
      if juice[:stock] >= 1 && @slot_money >= juice[:price]
  ...
  def buy(juice) # 購入操作 文字列を数字に入れ替えることも視野に入れてます
    ...
    when "cola"
      if @slot_money >= 120 && @juice[0][:stock] >= 1
        ...
        @sales += 120
        @slot_money -= 120
        name_and_change = [@juice[0][:name], @slot_money]
      else
      nil #何もしない
      end
    when "water"
      if @slot_money >= 100 && @juice[1][:stock] >= 1
        ...
        @sales += 100
        @slot_money -= 100
        name_and_change = [@juice[1][:name], @slot_money]
      else
      nil #何もしない
      end
    when "redbull"
      if @slot_money >= 200 && @juice[2][:stock] >= 1
        ...
        @sales += 200
        @slot_money -= 200
        name_and_change = [@juice[2][:name], @slot_money]
      else
      nil #何もしない
      end
    end
...
# ①irbを立ち上げる
# require '/Users/sakitayu/workspace/work_vending_machine/kansei3.rb'
...
# ⑥購入操作で返り値に釣り銭を出力する (→コーラ/水は１つずつ購入、redbullだけ6つ購入して6つめの時にnilを出力そして在庫を0にしてます)
# vm.buy("cola")
# vm.buy("water")
# vm.buy("redbull")
# vm.buy("redbull")
...
# ⑧自動販売機に今現在入っているいお金を表示する（480になります）
# vm.return_money
